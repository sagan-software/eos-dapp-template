...
class dapp_token_tester : public tester
{
 public:
   dapp_token_tester()
   {
      ...
      create_accounts({N(alice), N(bob), N(carol), N(dapp.token)});
      produce_blocks(2);

      set_code(N(dapp.token), dapp::testing::contracts::token_wasm());
      set_abi(N(dapp.token), dapp::testing::contracts::token_abi().data());

      produce_blocks();

      const auto &accnt = control->db().get<account_object, by_name>(N(dapp.token));
      abi_def abi;
   ...
   fc::variant get_stats(const string &symbolname)
   {
      ...
      vector<char> data = get_row_by_account(N(dapp.token), symbol_code, N(stat), symbol_code);
   ...
   fc::variant get_account(account_name acc, const string &symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account(N(dapp.token), acc, N(accounts), symbol_code);
                        ...
                        asset maximum_supply)
   {
      ...
      return push_action(N(dapp.token), N(create), mvo()("issuer", issuer)("maximum_supply", maximum_supply));
   }
...
BOOST_AUTO_TEST_SUITE(dapp_token_tests)

BOOST_FIXTURE_TEST_CASE(create_tests, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(create_negative_max_supply, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(symbol_already_exists, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(create_max_supply, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(create_max_decimals, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(issue_tests, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(retire_tests, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(transfer_tests, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(open_tests, dapp_token_tester)
try
{
...
BOOST_FIXTURE_TEST_CASE(close_tests, dapp_token_tester)
try
{
